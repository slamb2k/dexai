...
def log_request(method: str, url: str, user_id: Optional[str], status: str, details: Optional[Dict] = None):
    ...
    try:
        ...
        for chunk in response.iter_content(chunk_size=8192):
            ...
            if len(content) > max_size:
                ...
                return {
                    "success": False,
                    "error": f"Response exceeded max size: {max_size} bytes"
                }
        ...
        log_request(method, url, user_id, 'success', {
            'status_code': response.status_code,
            'elapsed_ms': elapsed_ms,
            'size': len(content)
        })
        ...
        log_request(method, url, user_id, 'failure', {'error': 'connection_error', 'details': str(e)})
