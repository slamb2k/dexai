...
class Session:
    ...
    def _build_message_with_history(self, content: str) -> str:
        ...
        Note: after a compaction, the SDK injects its own summary of earlier
        messages into the continued session. We do NOT re-inject history in
        that case — the SDK's summary is sufficient and adding our own would
        waste context window space. The PreCompact hook in hooks.py archives
        the full transcript for audit/debugging, but the session manager
        trusts the SDK's compacted context for conversation continuity.
