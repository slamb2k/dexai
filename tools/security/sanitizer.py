...
INJECTION_PATTERNS = [
    ...
    (r'forget\s+(everything|all|your)\s+(you\s+)?(know|learned|rules?)',
     'prompt_injection', 'high', 'Memory wipe attempt'),
    (r'disregard\s+(all\s+)?(previous|prior|your)\s+(instructions?|context)',
     'prompt_injection', 'high', 'Instruction disregard attempt'),
    (r'override\s+(your\s+)?(instructions?|programming|rules?)',
     'prompt_injection', 'high', 'Override attempt'),

    # Role manipulation
    (r'you\s+are\s+(now|actually)\s+(a|an|the)',
     'jailbreak', 'high', 'Role reassignment attempt'),
    (r'pretend\s+(to\s+be|you\s+are)\s+(a|an)',
     'jailbreak', 'medium', 'Role-playing manipulation'),
    (r'act\s+as\s+(if\s+)?(you|a|an)',
     'jailbreak', 'medium', 'Acting instruction'),
    (r'(imagine|suppose)\s+you\s+(have\s+)?no\s+(rules?|restrictions?|limits?)',
     'jailbreak', 'high', 'Restriction bypass attempt'),

    # DAN-style prompts
    (r'\bdan\b.*\bmode\b',
     'jailbreak', 'critical', 'DAN mode attempt'),
    (r'do\s+anything\s+now',
     'jailbreak', 'critical', 'DAN activation phrase'),
    (r'jailbreak(ed)?\s*(mode)?',
    ...
    (r'(show|tell|reveal|display|print|output)\s+(me\s+)?(your\s+)?(system\s+)?(prompt|instructions?)',
     'exfiltration', 'high', 'System prompt extraction'),
    (r'what\s+(are|is)\s+your\s+(system\s+)?(prompt|instructions?)',
     'exfiltration', 'medium', 'System prompt query'),
    (r'repeat\s+(your\s+)?(initial|system|original)\s+(prompt|instructions?)',
     'exfiltration', 'high', 'Prompt repeat request'),

    # Format exploits
    (r'```\s*(system|assistant|user)\s*\n',
     'prompt_injection', 'medium', 'Fake message format'),
    (r'^(system|assistant|user)\s*:',
     'prompt_injection', 'medium', 'Role prefix injection'),
    (r'\[/?INST\]',
     'prompt_injection', 'medium', 'Instruction tag injection'),
    (r'<\|?(system|assistant|user|endoftext)\|?>',
     'prompt_injection', 'high', 'Special token injection'),

    # Code injection (shell)
    (r';\s*(rm|mv|cp|cat|wget|curl|bash|sh|python|perl|ruby)\s',
     'code_injection', 'critical', 'Shell command injection'),
    (r'\$\([^)]+\)',
     'code_injection', 'high', 'Command substitution'),
    (r'`[^`]+`',
     'code_injection', 'medium', 'Backtick command'),
    (r'\|\s*(sh|bash|python)',
     'code_injection', 'high', 'Pipe to shell'),

    # Path traversal
    (r'\.\./\.\.',
     'code_injection', 'high', 'Path traversal'),
    (r'%2e%2e/',
     'code_injection', 'high', 'Encoded path traversal'),

    # SQL injection indicators
    (r"'\s*(or|and)\s*'?1'?\s*=\s*'?1",
     'code_injection', 'high', 'SQL injection pattern'),
    (r';\s*drop\s+(table|database)',
     'code_injection', 'critical', 'SQL drop command'),
    (r'union\s+(all\s+)?select',
     'code_injection', 'high', 'SQL union injection'),
    (r'--\s*$',
     'code_injection', 'low', 'SQL comment suffix'),
]
...
def main():
    ...
    parser.add_argument('--check', action='store_true',
                       help='Only check for issues, do not modify')
    ...
    parser.add_argument('--strip-html', action='store_true', default=True,
                       help='Strip HTML tags (default: true)')
    parser.add_argument('--no-strip-html', action='store_true',
                       help='Do not strip HTML tags')
    ...
    parser.add_argument('--no-pattern-check', action='store_true',
                       help='Skip injection pattern checking')
    ...
    if result.get('success'):
        status = "SAFE" if result.get('safe', True) or result.get('security', {}).get('risk_level') == 'none' else "RISK DETECTED"
        print(f"OK {status}")
    else:
