...
def get_connection() -> sqlite3.Connection:
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_transition_type ON transition_patterns(event_type)')
...
def get_default_buffer(event_type: str, user_id: Optional[str] = None) -> int:
    """
...
def get_learned_buffer(user_id: str, event_type: str) -> Optional[float]:
    """
...
def calculate_reminder_time(
    ...
    if reminder_dt < now:
        ...
        if event_dt > now:
            ...
            return {
                "success": False,
                "error": "Event time is in the past"
            }
...
def set_defaults(
    ...
    for name, value in [('meeting', meeting), ('deep_work', deep_work),
                       ('admin', admin), ('social', social), ('appointment', appointment)]:
...
def get_upcoming_reminder(
    ...
    for event in events:
        try:
            ...
            if reminder_dt > now:
                if next_reminder is None or reminder_dt < datetime.fromisoformat(next_reminder['reminder_time']):
...
def main():
    ...
    parser.add_argument('--event-type', dest='event_type', default='meeting',
                       choices=VALID_EVENT_TYPES, help='Type of event')
    parser.add_argument('--actual-minutes', dest='actual_minutes', type=float,
                       help='Actual transition time in minutes')
    parser.add_argument('--on-time', dest='on_time', action='store_true',
                       help='Whether user was on time')
    parser.add_argument('--late', dest='late', action='store_true',
                       help='Whether user was late')

    # Defaults for set-defaults action
    parser.add_argument('--meeting', type=int, help='Meeting buffer in minutes')
    parser.add_argument('--deep-work', dest='deep_work', type=int, help='Deep work buffer in minutes')
