...
class AutomationRunner:
    ...
    def __init__(self):
        ...
        self.status_file = PROJECT_ROOT / runner_config.get('status_file', '.tmp/automation_status.json')
        ...
        while self.running:
            try:
                ...
                if result.get('processed', 0) > 0:
                    self._log(f"Notifications: Processed {result['processed']} "
        ...
        try:
            while self.running:
                try:
                    ...
                    trigger_id, context = await asyncio.wait_for(
                        trigger_queue.get(),
                        timeout=1.0
                    )
    ...
    async def start(self):
        ...
        if trigger_config.get('enabled', True) and trigger_config.get('file_watcher', {}).get('enabled', True):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health = {
            "healthy": status.get('actually_running', False),
            "components": {}
        }
        ...
        if status.get('components'):
            for name, comp_status in status['components'].items():
                health['components'][name] = {
                    ...
                    'healthy': comp_status.get('running', False) and comp_status.get('errors', 0) < 10
                }
