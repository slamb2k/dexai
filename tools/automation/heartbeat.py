...
from tools.automation import DB_PATH, CONFIG_PATH, HEARTBEAT_FILE
...
def parse_heartbeat_file(path: Optional[Path] = None) -> List[Dict[str, Any]]:
    """
...
def get_checks(enabled_only: bool = True) -> List[Dict[str, Any]]:
    ...
    if enabled_only:
        cursor.execute('SELECT * FROM heartbeat_checks WHERE enabled = 1 ORDER BY section, check_name')
    else:
...
def run_heartbeat() -> Dict[str, Any]:
    ...
    if not checks:
        return {
            "success": True,
            "skipped": True,
            "reason": "No enabled checks found"
        }
...
def main():
    ...
    parser.add_argument('--action', required=True,
                       choices=['parse', 'sync', 'list', 'run', 'status',
