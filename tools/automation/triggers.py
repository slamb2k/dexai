...
def create_trigger(
    ...
    if not job_id and not action:
        return {"success": False, "error": "Either job_id or action must be provided"}
...
def get_trigger(trigger_id_or_name: str) -> dict[str, Any] | None:
    """Get a trigger by ID or name."""
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT * FROM triggers WHERE id = ? OR name = ?", (trigger_id_or_name, trigger_id_or_name)
    )
    row = cursor.fetchone()
    conn.close()
...
def list_triggers(
    trigger_type: str | None = None, enabled: bool | None = None
) -> list[dict[str, Any]]:
        ...
        triggers.append(
            {
                ...
                "target": row["target"],
                "job_id": row["job_id"],
                "action": row["action"][:50] + "..."
                if row["action"] and len(row["action"]) > 50
                else row["action"],
                "enabled": bool(row["enabled"]),
                "last_fired": row["last_fired"],
                "fire_count": row["fire_count"],
...
async def fire_trigger(trigger_id: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    """
    ...
    if trigger["job_id"]:
        try:
            ...
            return {
                ...
                "message": "Trigger fired, job execution created",
            }
        except Exception as e:
            return {"success": False, "error": f"Failed to execute job: {e!s}"}
...
def setup_file_watcher(callback: Callable | None = None) -> Optional["Observer"]:
    """
