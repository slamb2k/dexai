...
try:
    from memory_db import add_daily_log as db_add_daily_log
    from memory_db import add_entry
except ImportError:
...
def append_to_daily_log(
    content: str, entry_type: str = "note", timestamp: bool = True, category: str | None = None
) -> dict[str, Any]:
    """
...
def write_to_memory(
    ...
    """
    results = {"success": True, "log_result": None, "db_result": None}

    # Append to daily log
    if log_to_file:
        log_result = append_to_daily_log(
            content=content, entry_type=entry_type, category=tags[0] if tags else None
        )
...
def sync_log_to_db(date: str | None = None) -> dict[str, Any]:
    """
    ...
    # Sync to DB
    result = db_add_daily_log(date=date, summary=summary, raw_log=content, key_events=key_events)
...
def main():
    ...
    parser.add_argument(
        "--update-memory", action="store_true", help="Append to MEMORY.md instead of daily log"
    )
    parser.add_argument("--section", default="key_facts", help="Section in MEMORY.md to append to")
