...
def get_connection() -> sqlite3.Connection:
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_snapshots_captured ON context_snapshots(captured_at)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_snapshots_user_captured ON context_snapshots(user_id, captured_at DESC)')
...
def capture_context(
    ...
    if ttl_hours:
        ...
        # Use config default
        config = load_config()
        retention_days = config.get('working_memory', {}).get('cleanup', {}).get('snapshot_retention_days', 7)
        if retention_days:
...
def cleanup_snapshots(
    ...
    if older_than:
        ...
        if not duration:
            conn.close()
            return {"success": False, "error": f"Invalid duration format: {older_than}. Use format like '7d', '24h', '30m'"}
...
def get_stats(user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Total snapshots
    cursor.execute(f'SELECT COUNT(*) as total FROM context_snapshots WHERE {base_condition}', params)
...
Examples:
    ...
    parser.add_argument('--trigger', choices=VALID_TRIGGERS,
                       help='What triggered the capture')
    ...
    parser.add_argument('--include-expired', action='store_true',
                       help='Include expired snapshots in results')
