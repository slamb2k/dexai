...
def get_connection() -> sqlite3.Connection:
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_patterns_type ON behavior_patterns(user_id, pattern_type)')
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_activity_events_user ON activity_events(user_id)')
...
def detect_avoidance_patterns(user_id: str, config: Dict[str, Any]) -> List[Dict]:
    ...
    for row in cursor.fetchall():
        ...
        patterns.append({
            ...
            'confidence': min(config.get('max_confidence', 0.95),
                            0.5 + row['postpone_count'] * config.get('confidence_boost_per_occurrence', 0.1))
        })
...
def analyze_patterns(user_id: str, since: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    for pattern in avoidance:
        if pattern['confidence'] >= 0.5:
            save_pattern(user_id, 'avoidance', {
                'task_id': pattern['task_id'],
                'task_name': pattern['task_name']
            }, pattern['confidence'], pattern['postpone_count'])
            patterns_saved += 1

    for pattern in routines:
        if pattern['confidence'] >= 0.5:
            save_pattern(user_id, 'daily_routine', {
    ...
    if bursts and bursts[0]['confidence'] >= 0.5:
        save_pattern(user_id, 'productive_burst', {
            'hours': bursts[0]['burst_hours']
        }, bursts[0]['confidence'], bursts[0]['total_bursts'])
        patterns_saved += 1
...
def get_avoidance(user_id: str) -> Dict[str, Any]:
    ...
    for pattern in avoidance:
        formatted.append({
            ...
            "suggestion": f"Want me to find the smallest first step for this?",
...
Examples:
    ...
    parser.add_argument('--action', required=True,
                       choices=['analyze', 'habits', 'avoidance', 'weekly', 'detect',
                               'record-task', 'record-activity'],
    ...
    parser.add_argument('--event', help='Event type for task events (completed, postponed, started, abandoned)')
