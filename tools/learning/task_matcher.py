...
def get_tasks_connection() -> Optional[sqlite3.Connection]:
...
def _import_energy_tracker():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "energy_tracker",
            Path(__file__).parent / "energy_tracker.py"
        )
...
def _import_pattern_analyzer():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "pattern_analyzer",
            Path(__file__).parent / "pattern_analyzer.py"
        )
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module
...
def get_peak_hours(user_id: str, day: Optional[str] = None) -> Dict[str, Any]:
...
def find_best_time_for_task(
    ...
    if not profile_result.get('sufficient_data', False):
        ...
        if config.get('fallback_behavior') == 'wait_for_data':
            return {
                ...
                "task_name": task_name,
                "suggestions": [],
                "message": config.get('messages', {}).get('insufficient_data',
            ...
            return {
                ...
                "task_name": task_name,
                "suggestions": [
                    {"hour": 9, "day": "any", "reason": "Morning hours tend to work well for focused work"},
    ...
    for offset in range(look_ahead):
        ...
        check_day = current_day if (current_hour + offset) < 24 else DAY_NAMES[(now.weekday() + 1) % 7]
    ...
    if peak_result.get('peak_hours'):
        ...
        if current_day in peak_hours and peak_hours[current_day]:
            for peak_hour in peak_hours[current_day]:
                if peak_hour > current_hour and not any(s['hour'] == peak_hour for s in suggestions):
                    suggestions.append({
                        "hour": peak_hour,
                        "day": current_day,
                        "energy_level": "peak",
...
def check_energy_match(user_id: str, task_type: str) -> Dict[str, Any]:
    ...
    if energy_diff >= 0:
        ...
        match_status = "good"
        message = config.get('messages', {}).get('good_match',
            "Good timing - {task_type} work matches your current energy.").format(task_type=task_type)
    elif energy_diff >= -0.2:
        # Slightly under but manageable
        match_status = "acceptable"
        message = f"Your energy is a bit lower than ideal for {task_type} work, but you can manage it."
    else:
        ...
        if best_time.get('suggestions'):
            ...
            message = config.get('messages', {}).get('energy_mismatch_warning',
                "This might be easier around {optimal_period}.").format(optimal_period=optimal_time)
        else:
        ...
        for task in pending[:10]:
            ...
            if current_score and current_score >= required_score * 0.8:
                suggestions.append({
                    ...
                    "task_type": task_type,
                    "reason": f"Energy matches and this is pending",
...
Task Types:
    ...
    parser.add_argument('--task-type',
                       choices=['creative', 'problem_solving', 'writing', 'admin',
                               'organizing', 'communication', 'learning', 'review'],
                       help='Type of task')
    parser.add_argument('--count', type=int, default=3,
                       help='Number of suggestions for suggest action')
    ...
    if args.action == 'best-time':
        if not args.task_type:
            print(json.dumps({"success": False, "error": "--task-type required for best-time action"}))
            sys.exit(1)
