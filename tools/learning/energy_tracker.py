...
def get_connection() -> sqlite3.Connection:
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_energy_obs_time ON energy_observations(observed_at)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_energy_profiles_user ON energy_profiles(user_id)')
...
def calculate_energy_score(signals: Dict[str, Any], config: Dict[str, Any]) -> Tuple[float, float]:
    ...
    if 'response_time_ms' in signals:
        benchmarks = config.get('response_time_benchmarks', {'fast': 2000, 'normal': 5000, 'slow': 10000})
    ...
    if 'message_length' in signals:
        benchmarks = config.get('message_length_benchmarks', {'short': 20, 'normal': 100, 'long': 300})
    ...
    if 'active_duration_s' in signals:
        benchmarks = config.get('session_duration_benchmarks', {'brief': 120, 'normal': 600, 'extended': 1800})
    ...
    for (day_of_week, hour), scores in aggregates.items():
        ...
        cursor.execute('''
            ...
            ON CONFLICT(user_id, day_of_week, hour) DO UPDATE SET
                ...
                last_updated = ?
        ''', (
            user_id, day_of_week, hour, avg_score, sample_count, datetime.now().isoformat(),
            avg_score, sample_count, datetime.now().isoformat()
        ))
        profiles_updated += 1
...
Examples:
    ...
    parser.add_argument('--lookback', type=int, default=30,
                       help='Days to look back for rebuild (default: 30)')
